...
export const searchTvShows = async (query, apiKey) => {
    ...
    try {
        // Trim API key to avoid 401s from accidental spaces
        const trimmedKey = apiKey.trim();
        const response = await fetch(`/api/tmdb/search?type=tv&key=${encodeURIComponent(trimmedKey)}&query=${encodeURIComponent(query)}`);
...
export const searchMovies = async (query, apiKey) => {
    ...
    try {
        // Trim API key to avoid 401s from accidental spaces
        const trimmedKey = apiKey.trim();
        const response = await fetch(`/api/tmdb/search?type=movie&key=${encodeURIComponent(trimmedKey)}&query=${encodeURIComponent(query)}`);
