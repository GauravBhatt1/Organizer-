...
export const searchTvShows = async (query, apiKey) => {
    ...
    try {
        const response = await fetch(`/api/tmdb/search?type=tv&key=${encodeURIComponent(apiKey.trim())}&query=${encodeURIComponent(query)}`);
...
export const searchMovies = async (query, apiKey) => {
    if (!query || !apiKey) return [];
    try {
        const response = await fetch(`/api/tmdb/search?type=movie&key=${encodeURIComponent(apiKey.trim())}&query=${encodeURIComponent(query)}`);
