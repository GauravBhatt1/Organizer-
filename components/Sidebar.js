...
import { DashboardIcon, MoviesIcon, TvIcon, UncategorizedIcon, SettingsIcon, LogoIcon, CloseIcon } from '../lib/icons.js';

const NavItem = ({ page, icon, isActive, onClick, count }) => html`
  <button
    onClick=${onClick}
  ...
  >
    ${isActive && html`<div className="absolute left-0 top-0 bottom-0 w-0.5 bg-brand-purple"></div>`}
...
const SidebarContent = ({ currentPage, setCurrentPage, onCloseMobile, uncategorizedCount }) => {
...
const Sidebar = ({ currentPage, setCurrentPage, isOpen, setIsOpen, uncategorizedCount }) => {
  return html`
    <${React.Fragment}>
