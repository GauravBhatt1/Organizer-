...
import { PlayIcon, CheckCircleIcon, ExclamationIcon } from '../lib/icons.tsx';
...
const MOCK_JOB_HISTORY: ScanJob[] = [
...
const StatCard: React.FC<{ title: string; value: number; }> = React.memo(({ title, value }) => (
...
const JobStatusIcon: React.FC<{ status: ScanJob['status'] }> = ({ status }) => {
    if (status === 'completed') return <CheckCircleIcon className="w-5 h-5 text-green-400" />;
    if (status === 'failed') return <ExclamationIcon className="w-5 h-5 text-red-400" />;
    return null;
};
...
const Dashboard: React.FC<DashboardProps> = ({ onMenuClick }) => {
    ...
    return (
        <div className="h-full flex flex-col">
            <Header title="Dashboard" onMenuClick={onMenuClick} actionButton={<Button onClick={startScan} icon={<PlayIcon />} disabled={!!currentJob}>Start New Scan</Button>} />
